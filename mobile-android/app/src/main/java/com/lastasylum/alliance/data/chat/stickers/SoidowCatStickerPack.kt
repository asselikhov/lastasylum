...
/**
 * Built-in sticker pack «Soidow cat» under [ASSET_FOLDER] as PNG files.
 * Wire format: whole message body is exactly `[[soidow_cat:<file_stem>]]` (no .png suffix).
 * Allowed stems must match the catalog on the server (`soidow-cat-stickers.const.ts`).
 */
object SoidowCatStickerPack : ChatStickerPack {
    const val PACK_KEY: String = "soidow_cat"
    override val packKey: String = PACK_KEY
    override val titleRes: Int = R.string.chat_stickers_pack_soidow_cat
