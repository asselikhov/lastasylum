...
/**
 * Built-in sticker pack «ОБЖОРЫ» under [ASSET_FOLDER] as PNG files.
 * Wire format: whole message body is exactly `[[obzhory:<file_stem>]]` (no .png suffix).
 * Allowed stems must match the catalog on the server (`obzhory-stickers.const.ts`).
 */
object ObzhoryStickerPack : ChatStickerPack {
    const val PACK_KEY: String = "obzhory"
    override val packKey: String = PACK_KEY
    override val titleRes: Int = R.string.chat_stickers_pack_obzhory
